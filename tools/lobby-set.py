#!/usr/bin/env python3
r"""Set the CUSTOM GAMES lobby's map and gametype directly, bypassing the UI's
...
# A DIRECT anchor for LobbySetMap - the function prologue, not a call site - from
# ate47's v3.3.0 DLL, src/dll/bocw-dll/systems/exported.cpp (removed from master
# and the 3.3.0 tag on 2026-09-05, commit 10569f8; recovered from parent 46b1d1e).
# The match address IS the function: no E8 to decode. Two virtues over the call
# site above: (1) it cannot suffer the "call site is not the function" bug, and
# (2) it depends only on the function's own bytes, not a caller's, so it is more
# likely to survive a build change. Used as a CROSS-CHECK: the call-site sig's
# decoded target and this address should be equal; if the call site fails to
# resolve, this is the fallback.
#   48 89 5C 24 10 57 48 83 EC 20  mov [rsp+10],rbx; push rdi; sub rsp,20
#   48 8B DA 8B F9 8B D1 33 C9     mov rbx,rdx (map); mov edi,ecx (lobby); ...
SIG_SET_MAP_PROLOGUE = (
    "48 89 5C 24 10 57 48 83 EC 20 48 8B DA 8B F9 8B D1 33 C9 E8 ? ? ? ? "
    "84 C0 0F 84 B1 00 00 00 8B D7 48 89 74 24 30 33 C9 E8 ? ? ? ? "
    "48 85 C0 75 08 8D 70 78"
)
...
# Narrow rights for the SCAN-ONLY path: read + query only, exactly the handle
# width gfscan has used all session without TAC flagging it. The wide
# ALL_ACCESS handle (VM_WRITE + CREATE_THREAD) is opened ONLY when actually
# calling a setter, so recon carries no more exposure than the proven-safe tool.
PROCESS_QUERY_INFORMATION = 0x0400
PROCESS_VM_READ = 0x0010
PROCESS_READ_ONLY = PROCESS_QUERY_INFORMATION | PROCESS_VM_READ
...
class Proc:
    def __init__(self, pid: int, write: bool = False):
        self.pid = pid
        # Scan/read only needs a narrow read handle (gfscan-class); the wide
        # handle is requested only when we intend to write + create a thread.
        access = PROCESS_ALL_ACCESS if write else PROCESS_READ_ONLY
...
def find_prologue(proc: Proc, base: int, size: int, sig: str, label: str) -> int | None:
    """Scan for a function-PROLOGUE signature. The match address IS the function -
    no E8 decode. Returns the single match, or None (and says why)."""
    hits = scan(proc, base, size, sig)
    print(f"  {label}")
    print(f"    pattern    {sig[:40]}...")
    if not hits:
        print("    prologue   NONE - this build's function bytes differ")
        return None
    if len(hits) > 1:
        print(f"    prologue   {len(hits)} matches {[hex(h) for h in hits[:4]]} - too loose, ignoring")
        return None
    print(f"    prologue   {hits[0]:#x}  (module+{hits[0] - base:#x})")
    return hits[0]
...
def self_test() -> int:
    ...
    # The prologue sig matches the function's own opening bytes DIRECTLY, so a
    # planted copy must be found at exactly its start (no E8 decode).
    rxp = compile_sig(SIG_SET_MAP_PROLOGUE)
    pro = bytes.fromhex(
        "48895C2410574883EC20488BDA8BF98BD133C9" "E8DDCCBBAA"
        "84C00F84B1000000" "8BD74889742430" "33C9" "E899887766"
        "4885C07508" "8D7078"
    )
    blob3 = bytearray(b"\x00" * 0x40) + bytearray(pro) + bytearray(b"\x00" * 0x40)
    check("SIG_SET_MAP_PROLOGUE finds a planted function entry",
          0x40 in [m.start() for m in rxp.finditer(bytes(blob3))])
    check("prologue sig length is 54 bytes (direct anchor, no rel32 to decode)",
          sig_len(SIG_SET_MAP_PROLOGUE) == 54)
...
def main() -> int:
    ...
    # Wide handle only when we will actually call a setter; scan-only stays read-only.
    proc = Proc(pid, write=bool(args.gametype or args.map))
    try:
        print("\nscanning")
        ...
        if args.func_map:
            ...
            # Cross-check the call-site target against the direct prologue anchor.
            fn_map_pro = find_prologue(
                proc, base, size, SIG_SET_MAP_PROLOGUE, "LobbySetMap (prologue)"
            )
            if fn_map is not None and fn_map_pro is not None:
                if fn_map == fn_map_pro:
                    print("    ✓ cross-check  call-site and prologue AGREE - high confidence")
                else:
                    print(f"    ⚠ cross-check  DISAGREE: call-site {fn_map:#x} vs prologue "
                          f"{fn_map_pro:#x} - do not call either until resolved")
                    fn_map = None  # refuse: two methods disagree = one is wrong
            elif fn_map is None and fn_map_pro is not None:
                print("    → call-site sig missed; using the prologue address as LobbySetMap")
                fn_map, map_targets = fn_map_pro, {fn_map_pro: []}
