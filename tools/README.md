...
| `exploders-gen.py` | Offline — bakes every map's `radiant_exploder` rows (`tables/bgcache/<map>.csv`, 699 over 32 maps) INTO `gunfight_menu.gsc` between its `[exploders-gen BEGIN/END]` markers as the walker's per-map table, cracking what it can against script string literals (34 named); also writes `docs/data/map-exploders.json`. `--check` says whether the block is stale. `docs/notes/destructibles.md` §7-8 |
| `gf-control/config_scan.py` | **Online, Windows, READ-ONLY** — sweeps the game's memory for the `GFCFG` marker `gunfight_menu.gsc` publishes (the live packed config + gf_oob + bot knobs) and resolves it back to dvar values; the app's Config -> **Load current** button so the fields reflect what the game actually has (in-game menu picks, a previous run), not just app defaults. The bridge is app->game only, so this is the one read path. `docs/notes/hud-and-control-app` |
...
| `gscc-inspect.py` | Offline — reads a Cold War `.gscc`/`.cscc` **without ACTS**: header, `#using` list, every import named (builtins + stock functions: how it draws, what it touches), its own functions, every string. For reviewing a community menu before anyone injects it, or checking a build's imports. Validated on the Atian build (exports resolve to its source names). `docs/notes/ecosystem-survey.md` §7 |
