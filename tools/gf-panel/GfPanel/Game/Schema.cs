    ...
    public static readonly SectionDef[] Sections =
    {
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_rounds_loadout", Label = "Loadout + sides every", Kind = SettingKind.Choice, Default = -1,
                        Choices = C(("Lobby's value", -1), ("1 round", 1), ("2 rounds", 2), ("3 rounds", 3), ("4 rounds", 4)),
                        Tip = "gf_rounds_loadout\nRounds before the shared loadout rotates AND the sides switch - stock couples both (gunfight.gsc onendround). The mod sets the live level var too so the pick takes without a restart." },
        ...
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_deathbarrier", Label = "Death barriers", Kind = SettingKind.Choice, Default = 0, Scope = "move", Eff = Eff.Live,
                        Choices = C(("Stock", 0), ("Off - hurt volumes disabled", 1), ("Off - hurt volumes deleted (this round)", 2), ("Off - hurt volumes sunk", 3)),
                        Tip = "gf_deathbarrier\nThe map's trigger_hurt kill volumes (the instant death off a ledge / in water / under the map) - NOT the restricted area, and god mode does not survive them. Disabled = triggerenable(0) (reversible). Deleted = gone until the next round. Sunk = moved 40000u down. Try in that order; the BARRIER debug line measures which one the engine honours." },
