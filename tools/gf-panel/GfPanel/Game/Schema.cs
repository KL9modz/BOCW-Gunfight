    ...
    public static readonly SectionDef[] Sections =
    {
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_match_minutes", Label = "Match time limit (other modes)", Kind = SettingKind.Choice, Default = -1, Scope = "mtime", Eff = Eff.Live,
                        Choices = C(("Lobby's value", -1), ("3 min", 3), ("5 min", 5), ("10 min", 10), ("15 min", 15), ("20 min", 20), ("30 min", 30), ("45 min", 45), ("60 min", 60), ("Unlimited", 0)),
                        Tip = "gf_match_minutes (klaze 2026-09-24)\nThe match clock of every mode EXCEPT Gunfight: TDM, Free-for-all, Kill Confirmed, Hardpoint ... In a round-based mode (Search & Destroy) it is each round's clock. Gunfight has no match clock - its length is Round timer + First to N rounds.\nLIVE: the mod runs the stock clock off this value instead of writing the lobby's Time Limit setting (the round timer's lever), so a change never reloads the match. A limit at or under the time already played gives 1 more minute instead of ending the match (End match is the instant end). Lobby's value = the lobby's Time Limit row. Modes with a clock of their own (Control, Infected and a few others) keep the lobby's row - the feed says so. Untested in-game." },
        ...
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_invwall", Label = "Invisible walls", Kind = SettingKind.Choice, Default = 1, Scope = "move", Eff = Eff.Live,
                        Choices = C(("Off - walk through them", 1), ("Stock - they block", 0)),
                        Tip = "gf_invwall\nThe map's player clip: the invisible ceiling in the sky and the walls around the edge (klaze 2026-09-24). Off (the default, klaze's call) = every living player holds engine perk 0xa2, which takes player clip out of their movement collision. MEASURED on Diesel: the ceiling passes, but some edge walls still block - those also carry utility clip (the zombie clip), which the perk cannot remove. Solid walls, floors, glass and vehicles still block; a vehicle keeps its own clip. Re-applied every spawn and every second. The BARRIER debug line shows walls: and iw: (players holding it / alive)." },
                new() { Dvar = "gf_melee", Label = "Melee", Kind = SettingKind.Choice, Default = 1, Scope = "move", Eff = Eff.Live, Group = "Melee & pings",
                        Choices = C(("On - stock", 1), ("Off for everyone", 0)),
                        Tip = "gf_melee\nOff = nobody can knife (stock's allow_melee switch, the one Prop Hunt props use; re-set every spawn and every second). Separately, each player's own melee is always off while their mod menu is open, so the menu's Back never knifes (klaze 2026-09-24)." },
                new() { Dvar = "gf_ping", Label = "Pings", Kind = SettingKind.Choice, Default = 0, Scope = "move", Eff = Eff.Live,
                        Choices = C(("Off for everyone", 0), ("On - stock", 1)),
                        Tip = "gf_ping\nOff (the default, klaze 2026-09-24) = nobody can ping - a pad's D-pad Left pinged while navigating the menu. The game has one ping switch for the whole match, not one per player. Read off the game's code - that the switch reaches other players' games is not yet measured." },
                new() { Dvar = "gf_parachute", Label = "Parachutes", Kind = SettingKind.Choice, Default = 1, Eff = Eff.Live, Group = "Air",
                        Choices = C(("Off - stock", 0), ("Everyone", 1), ("Host only", 2)),
                        Tip = "gf_parachute\nThe Fireteam free-fall + parachute on every map (klaze 2026-09-23): fall from height - a jump boost, a heli, flying - and deploy it. Everyone is the default (klaze 2026-09-24). A Fireteam mode's hidden setting makes the spawn call two player builtins (globallogic_spawn.gsc:674); the mod calls that pair itself per player per spawn, so no match reload. Lands on everyone alive within a second. The height a free-fall starts at is the engine's - unmeasured." },
        ...
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_wraith_mode", Label = "Seatless aircraft flight (Wraith...)", Kind = SettingKind.Choice, Default = 0, Eff = Eff.Live,
                        Choices = C(("Autopilot, direct if it stalls", 0), ("Autopilot only", 1), ("Direct only (noclip-style)", 2)),
                        Tip = "gf_wraith_mode\nAny spawned aircraft the game will not seat you in (the Wraith - BO3's AI helicopter - and whichever planes / streak helicopters are the same): walk up and hold interact, or Vehicles > Enter vehicle, and you fly it from a chase camera. The game's own entry is tried first, so aircraft you can normally get into keep their normal ride. Autopilot = its own helicopter movement flies it toward where you steer (banking, momentum); direct = it is moved like fly mode (exact, no banking). The default switches to direct only if the autopilot never gets it moving (once it has moved it, a wall or the ground stalling it does not switch). Hold interact again to leave. Read when a flight starts." },
            },
        },
        new()
        {
        ...
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_hint_self_on", Label = "Host's own hint bar", Kind = SettingKind.Toggle, Default = 1,
                        Tip = "gf_hint_self_on\nThe host's always-on hint bar: the menu controls while the menu is open, the forge controls while building, 'Open menu' (F + E, pad D-pad Up) when idle. 0 = off." },
        ...
        new()
        {
            ...
            Rows = new SettingDef[]
            {
                ...
                new() { Dvar = "gf_dbg_buttons", Label = "Button probe (BTN)", Kind = SettingKind.Toggle, Default = 0, Eff = Eff.Live, Tip = "gf_dbg_buttons\nThe host's feed prints one BTN line each time the set of held buttons changes: pad (controller used last), mv (movement signs forward,right) and every engine button bit that is down. Press Share / Options / the touchpad / PS and see which bits (if any) reach the game." },
        ...
        new()
        {
            ...
            Note = "Place-mode feel (forge session, 2026-09-20). Read live every tick of the forge loop - a change applies while you build. Default = free-walk: you move normally (WASD / stick) and the prop rides your crosshair onto surfaces. Controller: D-pad left/right = cycle, D-pad up/down = turn, bumpers = scale, fire = place (hold = spray), Square = grab/drop, Triangle = undo, R3 = exit. Keyboard/mouse (2026-09-24): wheel = cycle, right-click + wheel = turn, tactical/lethal = scale, left-click = place (hold = spray), F = grab/drop, R = undo, E = exit.",
            Rows = new SettingDef[]
            {
                new() { Dvar = "gf_forge_pin", Label = "Pin the player while placing", Kind = SettingKind.Toggle, Default = 0, Eff = Eff.Live,
                        Tip = "gf_forge_pin\n0 (default - klaze: 'i need to be able to move while placing props') = free-walk: WASD / stick moves YOU, the prop rides the crosshair. 1 = pinned in place with WASD driving the prop instead (the earlier scheme): W/S distance, A/D turn, jump/crouch height, ADS + W/S scale." },
                new() { Dvar = "gf_forge_movestep", Label = "Distance step", Kind = SettingKind.Int, Default = 6, Min = 1, Max = 30, Step = 1, Eff = Eff.Live, Tip = "gf_forge_movestep\nPinned mode only: units per tick of W/S (x2.5 with sprint held). Free-walk has no distance - the prop sits on the aimed surface." },
                new() { Dvar = "gf_forge_rotstep", Label = "Turn step", Kind = SettingKind.Int, Default = 3, Min = 1, Max = 30, Step = 1, Eff = Eff.Live, Tip = "gf_forge_rotstep\nFree-walk: one turn press = 5x this many degrees (D-pad up/down; right-click + wheel on keyboard); sprint held = a 45-degree snap. Pinned mode: degrees per tick of A/D (x2 with sprint)." },
                new() { Dvar = "gf_forge_scalestep", Label = "Scale step (% per tick)", Kind = SettingKind.Int, Default = 2, Min = 1, Max = 20, Step = 1, Eff = Eff.Live, Tip = "gf_forge_scalestep\nA scale press (bumpers / tactical-lethal) = 5x this percent; held past 0.3 s it keeps scaling by this much every frame (x3 with sprint). Pinned mode: ADS + W/S, per tick." },
