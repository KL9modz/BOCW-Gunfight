...
| **Fly bind** | Movement → *Fly bind: ADS/Tac + Melee* | TACTICAL + MELEE (keyboard / mouse) or ADS + MELEE (controller, klaze 2026-09-24 — a pad no longer opens the menu with ADS + Melee, D-pad Up does) toggles `fly_think` with the menu closed; **always on for every client-menu holder** (klaze 2026-09-24: `client_menu_grant` starts it each grant / spawn, `client_menu_revoke` stops it unless the host's own switch is on; switching it off while they hold the menu leaves it running); ignored while the menu is open, forging, or in a vehicle. Per player (menu target), kept across lives | whether holding Tactical starts a throw that Melee cancels cleanly (PHA ships the same combo) |
