...
| `projectiles.md` | **bullets into rockets (built 2026-09-17, never run):** `weapon_fired` → `magicbullet( w, eye, far, self )`, rate-gated (default 300 ms; "every shot" is the full-auto measurement), nine universal projectile weapons validated against `level.weaponnone`, homing via `missile_settarget`, optional trail FX. Tracers have no script API; beams are `.csc`-only. Test sheet inside |
