...
### F8 — 💥 in-match CRASH on the 386,799 net build: manually-spawned aircraft + round transition
2026-09-18 ~23:07: after klaze injected the consolidated 386,799 build (scope+F1+F3+F5+net+vehmode-dormant), the game crashed IN-MATCH on Miami/gunfight_3v3 at a round transition — NEW sig 0x91f84370 / C55D66DA (distinct from the frontend/LUA crashes). Isolation (bocw-c3): klaze had spawned an **aircraft from the Vehicles page** (`veh_spawn`, 250u ahead +25) that sat IN the ground; the Miami round transition then crashed — i.e. a **mod-spawned vehicle entity surviving `map_restart`**. Exonerates bocw-0f's anti-stack net (inspected: well-guarded, only relocates on real overlap) and vehicle-mode (dormant: `mod_spawn_vehicle` returns when `gf_vehmode<=0`). Fix (bocw-c3, in progress): spawn air vehicles above the head + **despawn mod-spawned vehicles on round end / map_restart**. Then a CRASH-CONFIRMATION run (spawn a vehicle, ride a round transition, no crash) before it becomes the standard payload.
**Recovery:** staged a crash-free SAFE fallback at the standard path — `gunfight_menu.gscc` rebuilt from commit **81fa13b** (365,638 B: scope fix kills the apply-restart; NO vehmode/net; old spawn code klaze ran all session). Crashy net build → `gunfight_menu.net-crashed-386799.bak.gscc`. klaze runs the safe build (apply-restart fixed) + avoids the Vehicles-page spawn until the vehicle fix lands; then the consolidated build returns with everything. ⚠ Lesson: a mod-spawned entity (vehicle) that outlives a round `map_restart` can crash the engine — clean up spawned entities on the round boundary.
